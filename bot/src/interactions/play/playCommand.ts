...
import { ErrorCodes, RealmError } from '../../errors'
...
export = {
  ...
  async execute(interaction: CommandInteraction) {
    ...
    else throw new RealmError({ code: ErrorCodes.UserNotInVoice })
...
async function loadAndPlay(prisma: PrismaClient, player: AudioPlayer) {
  ...
  const audioTrack = createAudioResource(createReadStream(path), {
